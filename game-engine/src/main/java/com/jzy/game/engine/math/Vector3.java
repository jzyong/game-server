...
public class Vector3 implements Serializable, Cloneable, Vector<Vector3> {
    ...
    public static boolean linesIntersect(double x1, double z1, double x2, double z2, double x3, double y3, double x4, double y4) {
        ...
        if (commonDenominator == 0) {
            ...
            if (collinearityTestForP3 == 0) {
                ...
                if (x1 >= x3 && x1 <= x4 || x1 <= x3 && x1 >= x4 || x2 >= x3 && x2 <= x4 || x2 <= x3 && x2 >= x4 || x3 >= x1 && x3 <= x2 || x3 <= x1 && x3 >= x2) {
                    if (z1 >= y3 && z1 <= y4 || z1 <= y3 && z1 >= y4 || z2 >= y3 && z2 <= y4 || z2 <= y3 && z2 >= y4 || y3 >= z1 && y3 <= z2 || y3 <= z1 && y3 >= z2) {
                        return true;
                    }
                }
            }
    ...
    public boolean equal(Vector3 vector3, float precision) {
        if (Math.abs(this.x - vector3.x) < precision &&Math.abs(this.y - vector3.y) < precision &&Math.abs( this.z - vector3.z) < precision) {
            return true;
        }
        return false;
    }
