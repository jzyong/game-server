...
public class NavMesh implements Serializable, Cloneable {
     ...
     * 创建三角形形 ，地图数据预处理 <br>
     *
     * @param nodeConnector 节点连接关系处理器
     * @param triangleBlocks 三角形连接关系
     * @param triangles 三角形顶点需要，依次三个序号为一组
     * @param vertices 顶点坐标
     * @param bufferOuter TriangleConnection ture
     * 非行走区原始多边缓存为inner多边形，外多边形为包围该三角形形的六边形 false outer多边形，内外相同
     * @param editor true为编辑器模式
     */
...
//            System.out.println("随机点数:" + randomPointNum);
     ...
     * @param center 中心点
     * @param radius 半径
     * @param minDistanceToCenter 离中心点的最小距离
     * @return
     */
